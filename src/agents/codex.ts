...
import { sanitizeServerKeys } from './json-utils.js';
...
export function buildNestedToml(
  ...
  for (let idx = 0; idx < serverNames.length; idx++) {
    ...
    if (server.env && typeof server.env === 'object') {
      ...
      for (const [ek, ev] of envEntries) emit(`env.${tomlKey(ek)}`, String(ev));
    }
    ...
    const unknownKeys = Object.keys(server)
      ...
      .sort((a, b) => a.localeCompare(b));
    for (const uk of unknownKeys) emit(tomlKey(uk), (server as Record<string, unknown>)[uk]);
