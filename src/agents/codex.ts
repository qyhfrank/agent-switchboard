...
export function mergeConfig(
	...
	try {
			if (Object.keys(otherTopLevel).length > 0) {
				otherToml = (tomlStringify as any)(otherTopLevel as any);
			}
	} catch {
		otherToml = "";
	}
...
export function buildNestedToml(
	mcpServers: Record<string, Omit<McpServerLike, "enable">>,
): string {
	const lines: string[] = [];
	...
	for (let idx = 0; idx < serverNames.length; idx++) {
		...
		const KNOWN_ORDER: (keyof McpServerLike)[] = [
			"command",
			"args",
			"url",
			"type",
		...
		const emit = (k: string, v: unknown) => {
			...
			const rhs = (tomlStringify as any).value ? (tomlStringify as any).value(v) : JSON.stringify(v);
			lines.push(`${k} = ${rhs}`);
		};
