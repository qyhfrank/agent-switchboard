...
test('loadConfigLayers reads user, profile, and project configs when present', () => {
  withTempAsbHome((asbHome) => {
    ...
    fs.writeFileSync(
      userPath,
      ['agents = ["claude-code", "opencode"]', '[commands]', 'active = ["cmd-user"]'].join('\n')
    );
    ...
    assert.deepEqual(result.user.config.agents, ['claude-code', 'opencode']);
...
test('buildMergedSwitchboardConfig applies precedence project > profile > user', () => {
  withTempAsbHome((asbHome) => {
    ...
    assert.deepEqual(merged.agents, ['user-agent']);
