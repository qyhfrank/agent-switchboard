...
import { buildNestedToml, CodexAgent, ensureTrustEntry, mergeConfig } from '../src/agents/codex.js';
...
// ---------------------------------------------------------------------------
// CodexAgent: sanitizes MCP server names (colons → dashes)
// ---------------------------------------------------------------------------

test('CodexAgent.applyConfig: replaces colons in MCP server names', () => {
  withTempHomes(() => {
    ...
    assert.ok(!content.includes('"context7:context7"'), 'coloned name should not appear');
    assert.match(content, /mcp_servers\.context7-context7/, 'coloned name should be sanitized');
    assert.match(content, /mcp_servers\.plain-name/, 'already-valid name should be preserved');
  });
});
