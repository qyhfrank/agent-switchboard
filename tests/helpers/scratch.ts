...
export function seedTree(root: string, files: Record<string, string | Buffer>): string {
...
/** Seed a plugin source checkout at <asbHome>/plugins/<namespace>/ (or under `parent`). */
export function seedSource(
  ...
  return seedTree(path.join(parent, namespace), files);
}

/** A marketplace source whose single catalogued entry lives in a subdirectory. */
export function seedMarketplace(
  ...
  seedSource(homes, namespace, prefixed);
}

/** A minimal valid SKILL.md body for a skill named `name`. */
export function skillDoc(name: string): string {
  return `---\nname: ${name}\ndescription: ${name} does a thing\n---\n\nBody of ${name}.\n`;
}

/** Run the CLI entry point in-process, capturing exit code and both streams. */
export async function runMain(argv: string[]): Promise<{ code: number; out: string; err: string }> {
  let out = '';
  let err = '';
...
export async function inCwd<T>(dir: string, body: () => Promise<T>): Promise<T> {
...
/** First report entry matching every field given in `match`. */
export function entryFor(
  report: Report,
  match: Partial<Pick<ReportEntry, 'app' | 'type' | 'id'>>
): ReportEntry | undefined {
  return report.entries.find(
    (entry) =>
      (match.app === undefined || entry.app === match.app) &&
      (match.type === undefined || entry.type === match.type) &&
