...
/**
 * Derived cache for external marketplace entries. Ported from the 0.4.35
 * marketplace-cache suite: pinning and ref resolution, credential safety,
 * refresh generation safety, ownership and symlink defense, and retirement of
 * the ASB_HOME predecessor cache.
 */
...
test('ref and sha pins must resolve to the same exact commit', async () => {
  await withScratchHomes(async (scratch) => {
    ...
    const v1Sha = writePluginVersion(remote, 'v1');
    const v2Sha = writePluginVersion(remote, 'v2');
    const request = requestFor(scratch, remote, { sha: v2Sha });
    ...
    assert.throws(() => materializeEntry(homes, { ...request, sha: v1Sha }), /pin mismatch/);
...
test('refresh touches materialized plugins only', async () => {
  await withScratchHomes(async (scratch) => {
    const homes = homesOf(scratch);
    const firstRemote = createGitFixture(scratch.root, 'first-remote');
    const secondRemote = createGitFixture(scratch.root, 'second-remote');
    writePluginVersion(firstRemote, 'v1');
    writePluginVersion(secondRemote, 'v1');
    const first = requestFor(scratch, firstRemote, { pluginName: 'first-plugin' });
    const second = requestFor(scratch, secondRemote, { pluginName: 'second-plugin' });
    const materialized = materializeEntry(homes, first);

    const result = refreshEntryCache(homes, 'catalog', first.marketplacePath, [first, second]);
...
test('refresh drops entries the source no longer declares', async () => {
  await withScratchHomes(async (scratch) => {
    const homes = homesOf(scratch);
    const remote = createGitFixture(scratch.root, 'plugin-remote');
    writePluginVersion(remote, 'v1');
    ...
    materializeEntry(homes, kept);
    const droppedEntry = materializeEntry(homes, dropped);

    const result = refreshEntryCache(homes, 'catalog', kept.marketplacePath, [kept]);
...
// ── ASB_HOME predecessor cache: read to retire, never written ─────────────

function compatibilityCacheRoot(scratch: ScratchHomes): string {
  return path.join(scratch.asbHome, 'state', 'marketplace-plugins');
}

/** Move a materialized entry into the layout a 0.4 peer would have left behind. */
...
test('a verified predecessor retires only after a successful replacement', async () => {
  for (const owner of ['unchanged-path', 'migrated-source'] as const) {
    await withScratchHomes(async (scratch) => {
      ...
      writePluginVersion(remote, 'v1');
      const legacySourceDir = path.join(scratch.asbHome, 'plugins', 'retire-source');
      const cacheSourceDir = path.join(homes.cacheHome, 'retire-source');
      const firstOwner =
        owner === 'unchanged-path' ? path.join(scratch.asbHome, 'catalog') : legacySourceDir;
