...
test('distributeCommands: cursor output is pure content without frontmatter', () => {
  withTempHomes(() => {
    ...
    updateLibraryStateSection('commands', () => ({ enabled: [cmdId], agentSync: {} }));
    distributeCommands();
...
test('skills targets include cursor', () => {
  const ids = getTargetsForSection('skills').map((t) => t.id);
  assert.ok(ids.includes('cursor'), 'cursor should support skills');
});
...
test('distributeSubagents: cursor output has allowlisted frontmatter fields only', () => {
  withTempHomes(() => {
    ...
    updateLibraryStateSection('agents', () => ({ enabled: [subId], agentSync: {} }));
    distributeSubagents();
...
test('rules targets include cursor', () => {
  const ids = getTargetsForSection('rules').map((t) => t.id);
  assert.ok(ids.includes('cursor'), 'cursor should support rules');
});
...
test('distributeRules: writes single asb-rules.mdc for cursor', () => {
  withTempHomes(({ agentsHome }) => {
    ...
    saveRuleState({ ...DEFAULT_RULE_STATE, enabled: ['hygiene', 'style'], agentSync: {} });
...
test('distributeRules: cursor cleanup does not delete non-library .mdc files', () => {
  withTempHomes(({ agentsHome }) => {
    ...
    saveRuleState({ ...DEFAULT_RULE_STATE, enabled: ['managed'], agentSync: {} });
    distributeRules();
...
test('distributeRules: cursor skips unchanged asb-rules.mdc', () => {
  withTempHomes(() => {
    ...
    saveRuleState({ ...DEFAULT_RULE_STATE, enabled: ['stable'], agentSync: {} });
...
// ---------------------------------------------------------------------------
// MCP: Cursor sanitizes server names (colons → dashes)
// ---------------------------------------------------------------------------

test('CursorAgent.applyConfig: replaces colons in MCP server names', () => {
  withTempHomes(({ agentsHome }) => {
    const cursorDir = path.join(agentsHome, '.cursor');
    ...
    agent.applyConfig({
      mcpServers: {
        ...
        'plain-name': { command: 'node', args: ['server.js'], type: 'stdio' },
      },
    });
    ...
    assert.ok(!names.includes('context7:context7'), 'coloned name should not appear');
    assert.ok(names.includes('context7-context7'), 'coloned name should be sanitized to dashes');
    assert.ok(names.includes('plain-name'), 'already-valid name should be preserved');
  });
});

test('CursorAgent.applyProjectConfig: sanitizes server names in project config', () => {
  withTempDir((dir) => {
    withTempHomes(() => {
      const agent = new CursorAgent();
      agent.applyProjectConfig(dir, {
        mcpServers: {
          'ns:srv': { command: 'echo', args: [], type: 'stdio' },
        },
      });

      type McpJson = { mcpServers: Record<string, unknown> };
      const written = loadJsonFile<McpJson>(agent.projectConfigPath(dir), { mcpServers: {} });
      const names = Object.keys(written.mcpServers);

      assert.ok(!names.includes('ns:srv'), 'coloned name should not appear in project config');
      assert.ok(names.includes('ns-srv'), 'should be sanitized in project config');
    });
  });
});
