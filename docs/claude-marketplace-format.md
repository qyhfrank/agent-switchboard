...
Removing a recorded plugin from the selection makes the next sync uninstall its user-scope installation. ASB also removes a marketplace it registered when no selected plugin needs it and no unrecorded installed plugin remains in it. Project and local installations remain outside ASB ownership, including installations with the same ref. Unresolved selections defer marketplace cleanup. A marketplace registered outside ASB is preserved. Plugins and marketplaces that predate these ownership records are not automatically adopted, even when selected. ASB leaves unrecorded installations unchanged.
