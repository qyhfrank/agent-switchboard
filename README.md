...
`asb plugin marketplace update [source]` refreshes remote source checkouts and only the external entries already present in the derived cache. Removing a configured source removes only that source's derived entries. `asb sync --update` performs the same refresh after source readiness and before distribution. `asb sync --dry-run` never clones or migrates a source; when readiness work is pending, it reports the action and target path instead of calculating a distribution preview from incomplete content. Otherwise it uses a temporary entry cache for any materialization needed to calculate the preview.
